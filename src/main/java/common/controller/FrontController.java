...
@WebServlet(
		...
		initParams = { 
			...
			value = "C:\\multicampus\\java-workspace\\RealEstate\\src\\main\\webapp\\WEB-INF\\command.properties")
		})
public class FrontController extends HttpServlet {
		...
		try {
			...
			if(set!=null) {
				for(Object key:set) {
					...
					String className = pr.getProperty(cmd);//"common.controller.IndexAction" << 클래스명이 들어옴
					if(className!=null) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//init을 수행하고 나면 바로 doGet이나 doPost를 실행함
